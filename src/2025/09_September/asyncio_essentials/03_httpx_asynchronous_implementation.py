"""
This module demonstrates downloading multiple images concurrently using asyncio,
httpx.AsyncClient, and aiofiles for asynchronous file writing, followed by a
synchronous edge-detection process using PIL.

Features:
----------
- Asynchronously download multiple images in parallel using asyncio.TaskGroup.
- Writes image files asynchronously to disk with aiofiles to avoid blocking the event loop.
- Applies a simple edge-detection algorithm to each image synchronously.
- Measures execution time of both downloading and processing using a flexible timer decorator.

Notes on Performance:
---------------------
- While aiofiles enables non-blocking file writes, in practice, the overhead of
  asynchronous I/O for local file writes can outweigh the benefits for small-to-medium
  files. For large files or network-mounted storage, asynchronous writes may be more
  beneficial.
- Converting the download process from synchronous requests to httpx.AsyncClient
  improves concurrency and network-bound performance.
- Image processing remains synchronous and CPU-bound, so it does not benefit from
  async I/O.


Implementation Details / Key Differences
----------------------------------------
- use of httpx.AsyncClient for concurrent requests
- use of aiofiles.open for concurrent writting of the images
"""
...
async def download_single_image(client: httpx.AsyncClient, url: str, idx: int) -> Path:
...
async def download_images(urls: Iterable[Path | str]) -> Iterable[Path]:
    async with httpx.AsyncClient() as client:
        async with asyncio.TaskGroup() as tg:
            tasks = [tg.create_task(download_single_image(client, url, idx)) for idx, url in enumerate(urls)]
